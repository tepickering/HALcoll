...
PACKAGENAME = metadata.get('package_name', 'halcoll')
DESCRIPTION = metadata.get('description', 'ML-based open loop focus/collimation tools for the MMT Observatory')
AUTHOR = metadata.get('author', 'T. E. Pickering')
AUTHOR_EMAIL = metadata.get('author_email', '')
LICENSE = metadata.get('license', 'unknown')
URL = metadata.get('url', 'http://github.com/te_pickering/HALcoll/')
__minimum_python_version__ = metadata.get("minimum_python_version", "3.7")

# Enforce Python version check - this is the same check as in __init__.py but
# this one has to happen before importing ah_bootstrap.
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write("ERROR: halcoll requires Python {} or later\n".format(__minimum_python_version__))
    sys.exit(1)
